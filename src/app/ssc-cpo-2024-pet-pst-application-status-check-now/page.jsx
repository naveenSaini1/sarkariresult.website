export default function RecruitmentInformation() {
  // This component displays recruitment information for SSC CPO PET PST 2024.
  // It includes details like title, post details, brief information, important dates,
  // and vacancy details. It also provides links to relevant resources.
  return (
    <div className="container mx-auto px-4 py-8">
      <nav aria-label="breadcrumbs" className="rank-math-breadcrumb">
        <p>
          <a
            href="https://pmsuryaghar.org.in"
            className="text-blue-500 hover:underline"
          ></a>

          <span className="last">
            SSC CPO Physical Efficiency Test (PET) and Physical Standards Test
            (PST) Admit Card 2024, Application Status for Physical Test,
            Download Region Wise Links Given Here
          </span>
        </p>
      </nav>
      <header className="entry-header">
        <h1 className="entry-title text-3xl font-bold text-gray-900">
          SSC CPO PET PST Admit Card 2024, Appication Status for Physical Test,
          Download Region Wise Links Given Here
        </h1>
        <div className="entry-meta mt-2">
          <span className="posted-on">
            <time
              className="entry-date published"
              dateTime="2024-10-04T18:12:02+05:30"
            >
              October 4, 2024
            </time>
          </span>
          <span className="byline">
            by{" "}
            <span
              className="author vcard"
              itemProp="author"
              itemScope
              itemType="https://schema.org/Person"
            >
              <a
                className="url fn n text-blue-500 hover:underline"
                href="https://pmsuryaghar.org.in/author/pmsuryaghar/"
                title="View all posts by Admin"
                rel="author"
                itemProp="url"
              >
              
              </a>
            </span>
          </span>
        </div>
      </header>
      <div className="entry-content" itemProp="text">
        <p className="mt-4">
          <strong>SSC CPO PET PST Admit Card 2024</strong>: The Staff Selection
          Commission (SSC) released the CPO 2024 Tier-1 Result on 2 September
          2024. Now, the Physical Efficiency Test (PET) and Physical Standards
          Test (PST) for the Delhi Police and Central Armed Police Forces (CAPF)
          Sub-Inspectors will be conducted from 14 October 2024. The region-wise
          schedule and SSC CPO PET PST Admit Card, Application Status 2024
          Download links are given here.
        </p>
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO PET PST Admit Card 2024 Overview
        </h2>
        <figure className="wp-block-table mt-4">
          <table className="table-auto w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Recruitment Organization
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Exam Name
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="px-4 py-2">Staff Selection Commission (SSC)</td>
                <td className="px-4 py-2">SSC CPO 2024</td>
              </tr>
              <tr>
                <td className="px-4 py-2">Post Name</td>
                <td className="px-4 py-2">
                  Sub-Inspector in CAPFs and Delhi Police
                </td>
              </tr>
              <tr>
                <td className="px-4 py-2">Total Vacancies</td>
                <td className="px-4 py-2">4187</td>
              </tr>
              <tr>
                <td className="px-4 py-2">PET PST Date</td>
                <td className="px-4 py-2">14- 25 October 2024</td>
              </tr>
              <tr>
                <td className="px-4 py-2">Category</td>
                <td className="px-4 py-2">
                  <strong>SSC CPO PET PST Admit Card 2024</strong>
                </td>
              </tr>
              <tr>
                <td className="px-4 py-2">Official Website</td>
                <td className="px-4 py-2">ssc.gov.in</td>
              </tr>
            </tbody>
          </table>
        </figure>
      
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO 2024 Important Dates
        </h2>
        <p className="mt-4">
          The SSC CPO 2024 Notification was released on 4 March 2024 and the
          online applications were invited from 4 March to 28 March 2024. The
          SSC CPO 2024 Tier-1 CBT exam was conducted on 27, 28, and 29 June
          2024.
        </p>
        <p className="mt-4">
          The SSC has released the PET PST Schedule and Admit Cards for the
          Physical Tests of SSC CPO 2024. The SSC CPO 2024 PET and PST will be
          held from 14 to 25 October 2024.
        </p>
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO 2024 Vacancies, Eligibility
        </h2>
        <p className="mt-4">
          <strong>
            <span className={{ textDecoration: "underline" }}>Age Limit</span>
          </strong>
          : The age limit for the SSC CPO 2024 is 20-25 Years (As on 1 August
          2024).
        </p>
        <figure className="wp-block-table mt-4">
          <table className="table-auto w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Post Name
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Vacancy
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Qualification
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="px-4 py-2">
                  Sub-Inspector (CAPF/ Delhi Police)
                </td>
                <td className="px-4 py-2">4187</td>
                <td className="px-4 py-2">Graduate</td>
              </tr>
            </tbody>
          </table>
        </figure>
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO 2024 Selection Process
        </h2>
        <p className="mt-4">
          The <strong>SSC CGL 2024</strong> selection process includes the
          Tier-1 CBT exam followed by the Physical Efficiency Test (PET),
          Tier-II CBT Exam, Document Verification and Medical Examination.
        </p>
        <ol className="list-decimal ml-4 mt-4">
          <li>Tier-2 CBT Exam</li>
          <li>Physical Efficiency Test (PET) and PMT</li>
          <li>Tier-2 CBT Exam</li>
          <li>Document Verification</li>
          <li>Medical Examination</li>
        </ol>
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO 2024 PET and PMT
        </h2>
        <p className="mt-4">
          The candidates who will qualify the SSC CPO Tier-1 exam will be called
          for the Physical Efficiency Test (PET) and Physical Standards Test
          (PST).
        </p>
        <p className="mt-4">
          The details of the SSC CPO PET and PST are given below.
        </p>
        <figure className="wp-block-table mt-4">
          <table className="table-auto w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Male
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Female
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="px-4 py-2">100 m Race in 16 Seconds</td>
                <td className="px-4 py-2">100 m Race in 18 Seconds</td>
              </tr>
              <tr>
                <td className="px-4 py-2">1.6 Km Race in 6.5 Minutes</td>
                <td className="px-4 py-2">800 m Race in 4 Minutes</td>
              </tr>
              <tr>
                <td className="px-4 py-2">Long Jump 3.65 m</td>
                <td className="px-4 py-2">Long Jump 2.7 m</td>
              </tr>
              <tr>
                <td className="px-4 py-2">High Jump 1.2 m</td>
                <td className="px-4 py-2">High Jump 0.9 m</td>
              </tr>
            </tbody>
          </table>
          <figcaption className="wp-element-caption">
            SSC CPO 2024 PET
          </figcaption>
        </figure>
        <h2 className="wp-block-heading text-2xl font-bold text-gray-900 mt-8">
          SSC CPO PET PST Admit Card 2024 Link
        </h2>
        <p className="mt-4">
          The SSC CPO 2024 PET PST Admit Card download links are given below
          region-wise.
        </p>
        <figure className="wp-block-table mt-4">
          <table className="table-auto w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Region
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Status Link
                </th>
                <th className="px-4 py-2 text-gray-900 font-bold uppercase text-sm">
                  Admit Card
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://sscnr.nic.in/newlook/site/index.html"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>NR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.sscnwr.org/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>NWR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.ssc-cr.org/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>CR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="http://www.sscsr.gov.in/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>SR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.sscwr.net/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>WR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="http://www.sscer.org/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>ER</strong>
                  </a>
                </td>
                <td className="px-4 py-2">
                  <a
                    href="https://pmsuryaghar.org.in/wp-content/uploads/2024/10/SSC-CPO-2024-Application-Status-for-ER-Region.pdf"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>Status/ Venue</strong>
                  </a>
                </td>
                <td className="px-4 py-2">
                  <a
                    href="https://www.sscer.org/ac/sicpo2024_pet_pst_call_letter/DAC/dac"
                    target="_blank"
                    rel="noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>Admit Card</strong>
                  </a>
                </td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.sscmpr.org/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>MPR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.ssckkr.kar.nic.in/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>KKR</strong>
                  </a>
                </td>
                <td className="px-4 py-2">
                  <a
                    href="https://ssckkr.kar.nic.in/sschallticket/sicpo24_petstatusz.aspx"
                    target="_blank"
                    rel="noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>Status</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
              <tr>
                <td className="px-4 py-2">
                  <a
                    href="https://www.sscner.org.in/en/"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>NER</strong>
                  </a>
                </td>
                <td className="px-4 py-2">Status</td>
                <td className="px-4 py-2">Admit Card</td>
              </tr>
            </tbody>
          </table>
        </figure>
        <figure className="wp-block-table mt-4">
          <table className="table-auto w-full text-left border-collapse">
            <tbody>
              <tr>
                <td className="px-4 py-2">
                  SSC CPO 2024 <strong>Notification</strong>
                </td>
                <td className="px-4 py-2">
                  <a
                    href="https://pmsuryaghar.org.in/wp-content/uploads/2024/08/SSC-CPO-2024-Notification.pdf"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>Notification</strong>
                  </a>
                </td>
              </tr>
              <tr>
                <td className="px-4 py-2">SSC Official Website</td>
                <td className="px-4 py-2">
                  <a
                    href="https://ssc.gov.in/"
                    target="_blank"
                    rel="noopener"
                    className="text-blue-500 hover:underline"
                  >
                    <strong>SSC</strong>
                  </a>
                </td>
              </tr>
            </tbody>
          </table>
        </figure>
      </div>
    </div>
  );
}
